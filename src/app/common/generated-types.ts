...
export namespace GetCollection {
  ...
  export type Assets = NonNullable<(NonNullable<NonNullable<(NonNullable<(NonNullable<(NonNullable<GetCollectionQuery['collection']>)['productVariants']>)['items']>)[number]>['assets']>)[number]>;
  export type Product = (NonNullable<NonNullable<(NonNullable<(NonNullable<(NonNullable<GetCollectionQuery['collection']>)['productVariants']>)['items']>)[number]>['product']>);
}
...
export type GetCollectionQuery = (
  ...
  & { collection?: Maybe<(
    ...
    & { featuredAsset?: Maybe<(
      ...
      & { items: Array<(
        { __typename?: 'ProductVariant' }
        & Pick<ProductVariant, 'productId' | 'priceWithTax'>
        & { assets: Array<(
          { __typename?: 'Asset' }
          & Pick<Asset, 'id' | 'preview'>
        )>, product: (
          { __typename?: 'Product' }
          & Pick<Product, 'slug' | 'name' | 'customFields'>
        ) }
      )> }
    ) }
  )> }
